import BH_ProductUOMConversion as UOM
			...
			if getCategoryName is not None:
				if productFamily == "ALS":
					if region.upper() == "North America".upper():
						...
						discountThresholdQuery = "SELECT DISCOUNT_THRESHOLD FROM ALS_DISCOUNT_TABLE_FLOOR_PRICE_CALCULATION WHERE REVENUE_TYPE = '" + str(item.ProductTypeName) +"'AND PRODUCT_FAMILY = '" + str(getCategoryName.DIR_NAME) + "' AND COUNTRY = '{0}'".format(quoteCountry)
				if productFamily == "OIC":
					discountThresholdQuery = "SELECT * FROM OIC_FLOOR_PRICE WHERE MATERIAL_NUMBER ='" + str(item.PartNumber) +"' AND PRODUCT_TYPE = '" + str(item.ProductTypeName) +"' AND AREA = '{0}'".format(area)
				if productFamily == "CWI":
					...
					if context.Product.Attributes.GetByName('CWI Pricing Cond Type') is not None:
						item['PricingConditionType'] = context.Product.Attributes.GetByName('CWI Pricing Cond Type').SelectedValue.ValueCode
						item['BH_PricingCondTypeDesc'] = context.Product.Attributes.GetByName('CWI Pricing Cond Type').GetValue()
					discountThresholdQuery = "SELECT DISCOUNT_THRESHOLD FROM CWI_DISC_TABLE_FLOOR_PRICE_CALC WHERE REVENUE_TYPE = '" + str(item.ProductTypeName) +"'AND PROD_FAMILY = '" + str(item['Category_Name']) + "' AND AREA = '{0}'".format(area)
				Trace.Write("FPC DISCOUNT QUERY : " + discountThresholdQuery)
				discountThreshold = SqlHelper.GetFirst(discountThresholdQuery)
				if discountThreshold is not None:
					#Trace.Write("FPC DISCOUNT THRESHOLD : " + str(discountThreshold.DISCOUNT_THRESHOLD))
